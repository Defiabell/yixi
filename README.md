...
| `/surf` | you | the Surf (渡) ten-minute flow: name the urge, put the phone down, let it pass, then decide again |
| `/surf/review` | you | Surf's 30-day look-back: urge counts, the passed/opened ratio, body states and triggers |
| `/surf/setup` | you | edit the trigger chips `/surf` offers, plus the home-screen and Shortcut entry instructions |
...
| Tests | 820 tests over 36 files (Vitest + `@cloudflare/vitest-pool-workers`) |
...
src/ui/surf.ts      /surf — the ten-minute urge-surfing flow, one document, five steps
src/ui/surfreview.ts  /surf/review — the 30-day look-back over recorded urges
src/ui/surfsetup.ts  /surf/setup — edit the trigger chips, plus the home-screen and Shortcut entry points
